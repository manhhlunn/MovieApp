...
import com.android.movieapp.models.network.res2Detail
import com.android.movieapp.models.network.resDetail
...
class MediaRequest(private val mediaService: MediaService) {
    ...
    suspend fun getOMovieDetail2(
        slug: String,
    ): NetworkResponse<OMovieDetail> {
        disableSSLVerification()
        return mediaService.request<OMovieDetailResponse2>(
            url = "${MovieApp.baseURL}/phim/$slug.json"
        ).res2Detail()
    }
        ...
        try {
            ...
            fids.asyncMapIndexed { index, fileList ->
                ...
                val sources = "sources\\s*=\\s*(.*);".toRegex().find(player)?.groupValues?.get(1)
                    ?: return@asyncMapIndexed
                val qualities =
                    "quality_list\\s*=\\s*(.*);".toRegex().find(player)?.groupValues?.get(1)
                        ?: return@asyncMapIndexed
                listOf(sources, qualities).forEach {
                    fromJson<List<ExternalSources>>(it)?.forEach org@{ source ->
                        if (source.m3u8Url != null && source.m3u8Url.contains(".m3u8")
                                .not()
                        ) return@org
                        srcLinks.add(
                            SourceLink(
                                source.label ?: "External [Server ${index + 1}]",
